...
Program Name           : needle_panel_1.sas

Path                   : sg1/041_needle_panel/needle_panel.sas
                                  
Purpose                : To demonstrate the use of SAS 9.2 to generate quick plots
                          
Input Datasets/Views   : sashelp.stocks
...
*-------------------------------------------------------------------------------------
 ...
 %let Program_  = needle_panel;
 %let Style_RTF = US_Landscape_SG_Color; 
 %let Source_   = sashelp.stocks;
 ...
 *-----------------------------------------------------------------------------------  
  ...
  -----------------------------------------------------------------------------------*;
    %macro titles(pdf=Y);
       ...
       title3 'Figure 14-1.1.2. Stock Trend';
       title4 '(In US dollars)';
 ...
 *---------------------------------------------------------------------------------  
  ...
  %macro loop;
      proc sgpanel data=sashelp.stocks;
        panelby stock/novarname spacing=2 uniscale=row rows=1 columns=3;
        needle x=date y=close/lineattrs=(color=blue) ;
        colaxis fitpolicy=rotate;
        where date > "31dec2004"d ;
      run;          
  %mend loop;
