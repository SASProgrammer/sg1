"""build_data.py -- Example 033: Same data as 032. Panels stacked in portrait layout."""
import numpy as np, pandas as pd
rng=np.random.default_rng(seed=33); N_SUBJ=60; TRTS=["Placebo","Drug A 10mg","Drug A 25mg"]; PANELS=["Visit 1","Visit 2","Visit 3"]
trt_int={"Placebo":0,"Drug A 10mg":6,"Drug A 25mg":12}; panel_shift={"Visit 1":0,"Visit 2":5,"Visit 3":10}
records=[]; subj=1
for panel in PANELS:
    for trt in TRTS:
        for _ in range(N_SUBJ):
            x=rng.uniform(10,80); y=.65*x+trt_int[trt]+panel_shift[panel]+rng.normal(0,9)
