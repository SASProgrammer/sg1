# graph.R -- Example 034: Scatter panels, consistent Y. Run build_data.R first.
...
p<-ggplot(df,aes(x=x_val,y=y_val,color=trt,shape=trt))+
  ...
  facet_wrap(~panel_label,nrow=1,scales="free_x")+  # Y consistent, X free
  coord_cartesian(ylim=range(df$y_val))+
  labs(x="Baseline (x)",y="Endpoint (y)",color=NULL,shape=NULL,title="Scatter Panels — Consistent Y Axis")+
...
ggsave("034_scatter_panel_Y_consistent_linear.pdf",plot=p,width=14,height=6)
ggsave("034_scatter_panel_Y_consistent_linear.png",plot=p,width=14,height=6,dpi=150)
message("Saved 034.")
