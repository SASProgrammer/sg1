...
Program Name           : scatter_panel_Y_consistent_linear_1.sas 

Path                   : sg1/034_scatter_panel_Y_consistent_linear

Purpose                : To demonstrate creation of a scatter panel Y consistent linear
...
*-------------------------------------------------------------------------------------
 ...
 %let Program_  = scatter_panel_Y_consistent_linear_1;
 ...
 *-----------------------------------------------------------------------------------  
  ...
  -----------------------------------------------------------------------------------*;
   %macro titles(pdf=Y);
      ...
      title4 "Multi-Panel With Consistent Y axis";
 ...
 %macro loop;
   proc sgscatter data=sashelp.iris;*(where=(species="Virginica"));   
        compare x=(sepallength petallength)
                y=(petalwidth) / reg spacing=5 group=species grid;
   run;          
 %mend loop;
