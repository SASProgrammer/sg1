...
Program Name           : 006_median_q1q3_1.sas  

Path                   : sg1/006_median_q1q3

Purpose                : To demonstrate creation of a median plots
                                                    
Study Number(s)        : N/A 

Program Type           : SAS 9.4  
                           
Production Outputs     : &Program..rtf  - CSR in text table output   
                         &Program..pdf  - Appendix graph 
                         
Qualification Outputs  : medians.sas7bdat - validation output    
...
*-----------------------------------------------------------------------------------
 ...
 libname ods_ 'sg1\000_ods_template' access=readonly;
...
*-------------------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------------------;
 title1 j=r h=1.0 'Avidity Biosciences';
 title2 j=r h=1.0 'Study AV-US-xxx-xxx';
 title3 h=2.0 bold 'Figure 14-1.1.2. Laboratory Data by Visit ';
 title4 h=2.0 bold " Median Ambisome~{unicode '00AE'x} by Study Week";
...
*-----------------------------------------------------------------------------------
 ...
 proc means data=labs nway noprint;
    var lbstresn;
    output out=Dir_.medians n=n median=median q1=q1 q3=q3;
    by lbdesc visit trtgroup;
 run;

 proc print data=Dir_.medians;   
    var lbdesc visit trtgroup n median q1 q3; 
 run;
...
*-----------------------------------------------------------------------------------
 ...
 proc template;
    ...
    begingraph;
      ...
      scatterplot y=median x=visit_off /
                 group=trtgroup index=trtcd  markerattrs=(size=0) 
                 yerrorlower=q1  yerrorupper=q3 
                 errorbarattrs=(pattern=1 thickness=1px);

      seriesplot y=median x=visit_off / primary=true
  ...
  entryfootnote halign=left textattrs=(size=7pt)
               "Vertical lines represent the cfirst and third quartiles around the median."
               / pad=(top=5px) ;
...
*----------------------------------------------------------------------------------  
 ...
 proc sgrender data=medians template=begingraph;
      dynamic _xlabel="Study Week" _ylabel="Median AmBisome ~{unicode '00AE'x} ";
 run;
